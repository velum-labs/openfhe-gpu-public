...
class Benchmark {
 public:
  ...
  void ModDownBench() {
    ...
    Run("ModDown", &Context::ModDown, from, to);
    ckks.context.is_moddown_fused = true;
    Run("FusedModDown", &Context::ModDown, from, to);
  }
  ...
  // void FullKeyswitchBench() {
  //   const auto key = ckks.GetRandomKey();
  //   const Ciphertext ct = ckks.GetRandomCiphertext();
  //   Ciphertext out;
  //   // ckks.context.EvalSquareAndRelin(ct, key);
  //   Run("FullKeySwitch", &Context::EvalSquareAndRelin, ct, key, out);
  // }
